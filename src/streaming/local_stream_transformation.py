...
from pyspark.sql.functions import (
    ...
    size, lit, struct, array
)
from pyspark.sql.types import StructType, StructField, ArrayType, DoubleType, LongType
GEOIP_DB_PATH = 'C:/fraud_detection_project/geoip2/GeoLite2-Country.mmdb'
...
def transform_streams():
    try:
        ...
        logger.exception("STOPPING as received keyboard interrupt")
...
def run_fraud_logic(batch_df, batch_id):
    ...
    context_df = get_context_aggregated_df(batch_df.sparkSession, batch_df=batch_df)

    if context_df is False:
        logger.warning(f"FRAUD LOGIC: Skipping batch {batch_id} due to context error")
        return

    print(f"Context df returned is format: ")
    context_df.show()

    print("joining the batch dataframe with context dataframe ")
    #enriched_df = batch_df.join(context_df, on="card_id", how="left")
    enriched_df = batch_df.alias("b").join(
        context_df.alias("c"),
        (col("b.card_id") == col("c.context_card_id")) &
        (col("b.timestamp") == col("c.context_timestamp")),
        how="left"
    )
    print(f"ENRICHED/JOINED DF: ")
    enriched_df.show()

    print(f"Apply fraud rules")
    # Apply rule logic
    flagged_df = apply_fraud_rules(enriched_df)

    context_cols_to_drop = [
        "ip_country_set","txn_count_last_10_min","total_spent_last_hour","agg_card_limit","context_card_id","context_timestamp"
    ]
    ...
    print(f"View the flagged dataframe: ")
    flagged_df.show()

    ALERT_COLUMNS = [
        "transaction_id", "user_id", "card_id", "amount", "timestamp",
        "ip_country", "channel", "user_email_masked", "merchant_name",
        "device_id", "ip_address", "session_id", "fraud_flag", "rule_triggered"
    ]

    fraud_df = flagged_df.filter("fraud_flag = True").select(*ALERT_COLUMNS)
    print("Write FRAUD txns to sink")
    (fraud_df
     .write
     .format("delta")
     .mode("append")
     .saveAsTable("silver.alerts")
     )

    print("Write txns to sink")
...
def apply_fraud_rules(df: DataFrame) -> DataFrame:
    ...
    rule_1 = (size(col("ip_country_set")) > lit(1))

    print("rule2")
    # RULE 2: More than 5 txns in 10 mins
    rule_2 = col("txn_count_last_10_min") > lit(5)
    print("rule3")
    # RULE 3: High value volume > 60% of card limit in last 1 hour
    rule_3 = (col("total_spent_last_hour") / col("agg_card_limit")) > lit(0.6)
    ...
    print("Constructing triggered rules array")
    triggered_rules = array(
        when(rule_1, lit("Sudden IP Country Change")).otherwise(None),
        when(rule_2, lit("High Txn Volume (10 min)")).otherwise(None),
        when(rule_3, lit("High Value Spending (1 hr)")).otherwise(None)
    )

    df = df.withColumn("rules_array", triggered_rules)

    # Filter out nulls from the array
    df = df.withColumn("rule_triggered", expr("filter(rules_array, x -> x is not null)"))

    # Derive fraud_flag from whether any rule triggered
    df = df.withColumn("fraud_flag", size(col("rule_triggered")) > 0)

    # Optional: convert array to comma-separated string
    df = df.drop("rules_array").withColumn(
        "rule_triggered", expr("concat_ws(', ', rule_triggered)")
    )

    return df


def get_context_aggregated_df(spark : SparkSession, batch_df : DataFrame):
    try:
        ...
        print("spark Union with current batch")
        unified_df = context_df.unionByName(batch_df)
        print("spark Union COMPLETE")
        if unified_df.limit(1).count() == 0:
        ...
        ####################
        history = unified_df.alias("history")
        current = batch_df.select("card_id", "timestamp").alias("current")
        joined = history.join(
            current,
            (col("history.card_id") == col("current.card_id")) &
            (col("history.timestamp") <= col("current.timestamp")) &
            (col("history.timestamp") >= expr("current.timestamp - interval 1 hour"))
        )
        # 4. Group by card_id and timestamp of current txn
        print("Computing aggregates per transaction")
        context_features = joined.groupBy("current.card_id", "current.timestamp") \
            .agg(
            collect_set("history.ip_country").alias("ip_country_set"),
            count(
                expr("CASE WHEN history.timestamp >= current.timestamp - interval 10 minutes THEN 1 END")
            ).alias("txn_count_last_10_min"),
            sum("history.amount").alias("total_spent_last_hour"),
            max("history.card_limit").alias("agg_card_limit")
        ) \
            .withColumnRenamed("card_id", "context_card_id") \
            .withColumnRenamed("timestamp", "context_timestamp")

        print("Returning context features")
        context_features.show()
        return context_features

        """logger.info(f"FETCHING CONTEXT: Grouping the Context Dataframe")
        print(f"Doing the aggregate")
        aggregated_df = (
            unified_df.groupBy("card_id")
            .agg(
                collect_set("ip_country").alias("ip_country_set"),
                sum(
                    expr("CASE WHEN timestamp >= now() - interval 10 minutes THEN 1 ELSE 0 END")
                    ).alias("txn_count_last_10_min"),
                sum(
                    expr("CASE WHEN amount / card_limit > 0.6 THEN 1 ELSE 0 END")
