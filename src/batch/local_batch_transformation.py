...
# 4. Mask all PII and sensitive data : non-reversible
# 5. Handle Nulls
...
from pyspark.sql.functions import (
    ...
    when, md5, concat_ws, current_timestamp, monotonically_increasing_id, expr
)
...
def transform_dims_to_silver():
    ...
    configs = load_local_transformation_configs()
    ...
    logger.info(f"TRANSFORMATION: Trying to Acquire Local Spark Session!!")
    ...
    for table in tables:
        ...
        sink_name = target_layer + "." + table_name
        ...
        success = upsert_dataframe(spark=spark, source_df=prepped_df, sink_table=sink_name, business_key=business_key, surrogate_key=surrogate_key)
        if success is False:
            ...
            if "email" in field.lower():
                ...
                df = df.withColumn(
                    ...
                    when(
                        ...
                        concat(
                            lit("***-***-"),
                            expr(f"substring({field}, length({field}) - 3, 4)")
                        )
                    ).otherwise(None)
                )
                print("done")
...
def upsert_dataframe(spark : SparkSession, source_df : DataFrame, sink_table : str, business_key : str, surrogate_key):
    try:
        ...
        source_df = source_df.withColumn(surrogate_key, monotonically_increasing_id())
